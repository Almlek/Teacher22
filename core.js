...
`;
  ...
  /* ─── Navigation ─── */
  const VIEW_TITLES = {
    home:'الهائل', new:'تحضير جديد', result:'الخطة الدرسية',
    archive:'الأرشيف', library:'المكتبة', settings:'الإعدادات',
    bank:'دروس واختبارات'
  };
  function showView(name) {
    $$('.view').forEach(v => { v.hidden = v.dataset.view !== name; });
    $('#appbarTitle').textContent = VIEW_TITLES[name] || 'الهائل';
...
${content}`;
    ...
    try {
      ...
      <div style="margin-top: 40px; text-align: center; padding-top: 15px; border-top: 2px dashed #cbd5e1; color: #64748b; font-size: 14px; font-weight: bold;">
          ✨ تم التوليد بواسطة المساعد الذكي - تطبيق الهائل
      </div>
      `;
  ...
  // ══════════════════════════════════════════════════════════════
  // 💡 دالة توليد "الموجه الشخصي" (استراتيجيات التدريس وتقويم الهائل)
  // ══════════════════════════════════════════════════════════════
  window.generateTeachingMentor = async function(e) {
    ...
    if(typeof showOverlay === 'function') showOverlay('جاري تحليل الدرس وتجهيز الاستراتيجيات وأدوات الهائل... 👨‍🏫');

    // 💡 البرومبت الهندسي الذي يربط الاستراتيجيات بأدوات تطبيق الهائل وتقنيات الأوفلاين
...
3. ربط ذكي وعملي بأدوات تطبيق (الهائل):
...
- (الإثراء والربط بالواقع 🌍): تطبيقات عملية للدرس من الحياة اليومية.
 - (الخرائط الذهنية🌍): خرائط ذهنية ينشئها تطبيق الهائل ويمكن ارسالها او عرضها للطلاب.
- (الملخص الشامل 🌍): ملخص منسق بشكل جميل يمكن ارساله للطلاب او طباعته.
- (تقويم الدرس او الوحدة 🌍): يقوم تطبيق الهائل بحل تقويم الدرس او الوحدة بشكل منسق ورائع ويمكن للمعلم مشاركته لطلابه.
...
- 🚀 **تلميح تطبيق الهائل:** (اربطها صراحة بإحدى أدوات التطبيق المذكورة أعلاه واشرح كيفية توظيفها وإرسالها).
...
- 🚀 **تلميح تطبيق الهائل:** (اربطها صراحة بأداة الاختبار الشامل أو الملخص التفاعلي، واذكر إرسالها الى هواتف الطلاب  وميزة التغذية الراجعة الفورية).
...
${content}`;
    ...
    try {
      ...
      <style>
          ...
          /* تمييز تلميحات تطبيق الهائل بوضوح شديد */
          .mentor-body li:has(strong:contains("تلميح تطبيق الهائل")) {
          ...
          .mentor-body li:has(strong:contains("تلميح تطبيق الهائل")) strong {
      ...
      <div style="margin-top: 40px; text-align: center; padding-top: 15px; border-top: 2px dashed #cbd5e1; color: #64748b; font-size: 14px; font-weight: bold;">
          👨‍🏫 تم إعداد هذا التوجيه بواسطة المساعد الذكي - تطبيق الهائل (Offline-First Ready)
      </div>
      `;
...
// ══════════════════════════════════════════════════════════════
  ...
  async function openGlobalSearch() {
      ...
      input.value = '';
      resultsDiv.innerHTML = '<div style="text-align: center; color: #94A3B8; font-weight: bold; margin-top: 50px; font-size: 1.2rem;">🔍 اكتب كلمة للبحث في جميع أقسام الهائل...</div>';
      modal.classList.add('is-active');
    ...
    document.addEventListener('deviceready', function () {
      document.addEventListener('backbutton', function (e) {
        ...
        if (navigator.app && navigator.app.exitApp) {
          if (confirm('هل تريد الخروج من تطبيق الهائل؟')) navigator.app.exitApp();
        }
      }, false);
    }, false);
